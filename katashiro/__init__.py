...
class Manager(object):
    def __init__(self, seq_factory, domain_factory, atom_factory, fields_factory=list):
        ...
        if self.is_atom(domain):
            ...
            for f in fields:
                ...
                if v is not None:
                    new_fields.append(v)
                    if v.id in metadata:
                        new_metadata[v.id] = metadata[v.id]
            return self.Domain(domain.id, new_fields, new_metadata)
    ...
    def _include_shallow(self, domain, predicate):
        ...
        for f in fields:
            if predicate(f):
                new_fields.append(f)
            if f.id in metadata:
                new_metadata[f.id] = metadata[f.id]
        return self.Domain(domain.id, new_fields, new_metadata)
    ...
    def _add_field(self, domain, field):
        self.domain.fields.append(field)

    def _add_metadata(self, domain, k, v):
        self.domain.metadata[k] = v
    ...
    def shortcut(self, id, attributes, metadata=None):
        r = self.fields_factory()
        for child in attributes:
            if isinstance(child, (list, tuple)):
                if len(child) < 3:
                    if isinstance(child[1], (list, tuple)):
                        # id, fields
                        r.append(domain(child[0], child[1]))
                    else:
                        sub_id = child[0]
                        sub_fields, sub_metadata = child[1].decompose()
                        r.append(self.Domain(sub_id, sub_fields, sub_metadata))
                else:
                    # id, fields, metadata
                    r.append(domain(child[0], child[1], child[2]))
            else:
                # id
                r.append(self.Atom(child))
        if self.is_seq(metadata):
            return self.Seq(id, r, metadata)
        else:
            return self.Domain(id, r, metadata)
...
class _Domain(object):
    ...
    def decompose(self):
        return self.fields[:], self.metadata.copy()
    ...
    def __getattr__(self, id):
        if self.cache is None:
            self.cache = {f.id: f for f in self.fields}
...
Person = (Domain("person")
          + Atom("name", {"datatype": S.str})
          + Atom("age", {"datatype": S.int})
          + Atom("birth", {"datatype": S.datetime}))
print(Person)
Parents = domain("parents", [("mother", Person), ("father", Person)])
print(Parents.exclude(lambda d: d.metadata.get("datatype") == S.datetime))
